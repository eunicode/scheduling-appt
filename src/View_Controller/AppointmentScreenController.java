...
public class AppointmentScreenController implements Initializable {
    ...
    ObservableList<String> viewByMonth = FXCollections.observableArrayList("January", "February","March","April","May","June","July","August","September","October","November","December");
    ObservableList<String> viewByWeek = FXCollections.observableArrayList("Previous Week", "This Week", "Next Week");
    ...
    void searchTableSorterHandler(ActionEvent event) {
        ...
        customerNameColumn.setCellValueFactory(new PropertyValueFactory<>("customerName"));
        customerContactColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getContact()));
        appointmentTitleColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getTitle()));
        appointmentLocationColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getLocation()));
        appointmentDescriptionColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getDescription()));
        appointmentStartColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getStart()));
        appointmentEndColumn.setCellValueFactory(appointment -> new SimpleStringProperty(appointment.getValue().getEnd()));
...
//            singleAppointment.forEach(allAppointments::remove); 
    ...
    public void deleteAppointment (Appointment appointment) {
        ...
        try {
            Statement statement = DBConnection.getConnection().createStatement();
                String deleteCustomer = "DELETE FROM appointment WHERE appointmentId = " + selectedID;
            ...
            if(deletedCustomer > 0) {
                ...
                System.out.println("Appointment record was successfully deleted from the database!");
    ...
    public void sortAppointment() {
            ...
            String getCustomerName = "SELECT customer.customerName FROM appointment JOIN customer ON customer.customerId = appointment.customerId " +
